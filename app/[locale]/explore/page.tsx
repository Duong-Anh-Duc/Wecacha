          ...
          <div className="flex h-full flex-col justify-between gap-5 lg:col-span-4">
            {listArticles.map((article, i) => (
              <Reveal key={i} delay={0.2 + i * 0.15}>
                <Link href={article.href} className="group flex h-auto flex-col items-start gap-4 rounded-[1.5rem] border border-[#142918]/5 bg-[#fdfcfb] p-2 shadow-[0_16px_50px_rgba(20,41,24,0.07)] transition-all hover:-translate-y-1 hover:shadow-[0_24px_70px_rgba(20,41,24,0.12)] sm:h-[172px] sm:flex-row sm:items-center sm:gap-6 sm:p-2.5 lg:h-[190px]">
                  <div className="relative h-48 w-full shrink-0 overflow-hidden rounded-[1rem] sm:h-full sm:w-[190px] lg:w-[200px] xl:w-[220px]">
                    <Image
                      src={article.image}
                      alt={article.title}
                      fill
                      className="object-cover transition duration-[1.5s] group-hover:scale-105"
                      sizes="(min-width: 1280px) 220px, (min-width: 640px) 190px, 100vw"
                    />
                  </div>
                  <div className="min-w-0 flex-1 py-2 pr-2 sm:pr-0">
                    <p className="mb-2 text-[9px] font-bold uppercase tracking-[0.24em] text-[#b5703a]">{article.date}</p>
                    <h3 className="mb-2 font-serif text-[1.45rem] leading-[1.18] text-[#142918] transition-colors group-hover:text-[#b5703a] lg:text-[1.65rem]">{article.title}</h3>
                    <p className="line-clamp-3 text-[12px] leading-relaxed text-[#142918]/60 sm:text-[13px]">{article.desc}</p>
                  </div>
                  <div className="hidden sm:flex h-9 w-9 shrink-0 mr-4 rounded-full bg-[#142918] items-center justify-center text-white transition-all duration-300 group-hover:bg-[#b5703a] group-hover:scale-110 shadow-sm">
